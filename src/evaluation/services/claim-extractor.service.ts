...
export class ClaimExtractorService {
  ...
  async extractClaims(answerText: string): Promise<Claim[]> {
    this.logger.debug(`Extracting claims from answer (${answerText.length} chars)`);
    ...
    try {
      const response = await this.ollamaService.chat([
        {
          role: 'system',
          content: 'You are a claim extraction expert. Extract discrete factual claims from text and identify substantive words. Always respond with valid JSON only, no additional text.',
        },
        {
          role: 'user',
          content: prompt,
      ...
      const claims = this.parseClaimsResponse(response.message.content, answerText);
...
[
  ...
  private parseClaimsResponse(responseContent: string, originalText: string): Claim[] {
    try {
      // Try to extract JSON from the response
      const jsonMatch = responseContent.match(/\[[\s\S]*\]/);
      if (!jsonMatch) {
        this.logger.warn('No JSON array found in response, attempting full parse');
      }
      ...
      return parsedClaims.map((claim) => {
        ...
        if (!claim.text || !claim.type || !Array.isArray(claim.substantiveWords)) {
        ...
        const sourceSpan = claim.sourceSpan || this.findSourceSpan(claim.text, originalText);
      ...
      }).filter(claim => claim !== null) as Claim[];
    } catch (error) {
  ...
  private findSourceSpan(claimText: string, originalText: string): { start: number; end: number } {
